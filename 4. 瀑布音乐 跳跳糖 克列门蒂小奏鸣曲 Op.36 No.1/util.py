...
def falling_block(x,y,z,vx,vy,vz,tick,NoGravity=0,block='wool',data=0):
  x,y,z    = x*1.0, y*1.0, z*1.0
  vx,vy,vz = vx*1.0,vy*1.0,vz*1.0
  return f'summon falling_block {x} {y} {z} {{Motion:[{vx},{vy},{vz}],NoGravity:{NoGravity},Time:{600-tick},DropItem:0b,Block:{block},Data:{data}}}'
...
class FallingBlock: 
  def __init__(self, x,y,z, x2,y2,z2, tick, noGravity=False, block='sand', data=0, toSet=False):
    self.x, self.y, self.z, self.x2,self.y2,self.z2, self.tick, self.noGravity = x,y,z, x2,y2,z2, tick, noGravity 
...
def fallingBlock(x,y,z, x2,y2,z2, tick, noGravity=False, block='sand', data=0, toSet=False):
  return FallingBlock(x,y,z, x2,y2,z2, tick, noGravity, block, data, toSet).toCmd()
